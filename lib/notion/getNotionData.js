...
export async function getGlobalNotionData({
  ...
  notionPageData.allNavPages = getNavPages({
    allPages: notionPageData.allPages
  })
...
function getLatestPosts({ allPages, from, latestPostCount }) {
  ...
  const latestPosts = Object.create(allPosts).sort((a, b) => {
    const dateA = new Date(a?.createdTime || a?.date?.start_date)
    const dateB = new Date(b?.createdTime || b?.date?.start_date)
    return dateB - dateA
  })
...
export function getNavPages({ allPages }) {
  ...
  const result = allNavPages.map(item => ({
    id: item.id,
    title: item.title,
  ...
  const groupedArray = result.reduce((groups, item) => {
    ...
    if (!lastGroup || lastGroup.category !== categoryName) {
      // 如果当前元素的category与上一个元素不同，则创建新分组
